...
generalists[,32] <- ifelse(generalists[,32] == "LC", 0,
                           ifelse(generalists[,32] == "NT", 0,
                                  ifelse(generalists[,32] == "VU", 1,
                                         ifelse(generalists[,32] == "EN", 1,
                                                ifelse(generalists[,32] == "CR", 1, NA)))))
...
generalists.gain.excluded <- generalists %>%
...
generalists.gain.excluded.na.omit<-na.omit(generalists.gain.excluded[,vars])

# Create data frame for extinction risk (ER) models
data.generalists<-data.frame(or=generalists.gain.excluded.na.omit$order,
			     hip5=generalists.gain.excluded.na.omit$habitat_intactness_p5,
			     hip10=generalists.gain.excluded.na.omit$habitat_intactness_p10,
			     hip50=generalists.gain.excluded.na.omit$habitat_intactness_p50,
			     hip90=generalists.gain.excluded.na.omit$habitat_intactness_p90,
			     hip95=generalists.gain.excluded.na.omit$habitat_intactness_p95,
			     rhip5=generalists.gain.excluded.na.omit$change_habitat_intactness_p5,
			     rhip10=generalists.gain.excluded.na.omit$change_habitat_intactness_p10,
			     rhip50=generalists.gain.excluded.na.omit$change_habitat_intactness_p50,
			     rhip90=generalists.gain.excluded.na.omit$change_habitat_intactness_p90,
			     rhip95=generalists.gain.excluded.na.omit$change_habitat_intactness_p95,
			     pip5=generalists.gain.excluded.na.omit$patch_intactness_p5,
			     pip10=generalists.gain.excluded.na.omit$patch_intactness_p10,
			     pip50=generalists.gain.excluded.na.omit$patch_intactness_p50,
			     pip90=generalists.gain.excluded.na.omit$patch_intactness_p90,
			     pip95=generalists.gain.excluded.na.omit$patch_intactness_p95,
			     rpip5=generalists.gain.excluded.na.omit$change_patch_intactness_p5,
			     rpip10=generalists.gain.excluded.na.omit$change_patch_intactness_p10,
			     rpip50=generalists.gain.excluded.na.omit$change_patch_intactness_p50,
			     rpip90=generalists.gain.excluded.na.omit$change_patch_intactness_p90,
			     rpip95=generalists.gain.excluded.na.omit$change_patch_intactness_p95,
			     rs=generalists.gain.excluded.na.omit$range_size,
			     gl=generalists.gain.excluded.na.omit$gestation_length,
			     wa=generalists.gain.excluded.na.omit$weaning_age,
			     rl.cat=generalists.gain.excluded.na.omit$rl_cat)
...
hip5.glmer<-glmer(rl.cat~hip5+rhip5+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
hip10.glmer<-glmer(rl.cat~hip10+rhip10+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
hip50.glmer<-glmer(rl.cat~hip50+rhip50+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
hip90.glmer<-glmer(rl.cat~hip90+rhip90+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
hip95.glmer<-glmer(rl.cat~hip95+rhip95+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
...
hi.effectsizes.table$habitat_breadth <- "Generalists"

#----
# ER ~ patch intactness (pi) + reduction in patch intactness (rpi) + ...
pip5.glmer<-glmer(rl.cat~pip5+rpip5+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
pip10.glmer<-glmer(rl.cat~pip10+rpip10+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
pip50.glmer<-glmer(rl.cat~pip50+rpip50+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
pip90.glmer<-glmer(rl.cat~pip90+rpip90+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
pip95.glmer<-glmer(rl.cat~pip95+rpip95+rs+gl+wa+(1|or), family=binomial, data=data.generalists.scaled)
...
pi.effectsizes.table$habitat_breadth <- "Generalists"

# Combine effect sizes for hi and pi together
hipi.effectsize.table<-rbind(hi.effectsizes.table, pi.effectsizes.table)

# Save effect sizes
write.table(hipi.effectsize.table, file="effectsize_hipi_generalists.csv", sep="\t", row.names=F)
